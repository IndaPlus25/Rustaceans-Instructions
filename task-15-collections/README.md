...
You think you know lists, but you don't (or maybe you do, who knows?). 

Prove it, in style. Your goal:
...
1) Create a repository named `task-15-<KTH_ID>`.
2) Clone your regular assignment repository.
    ```sh
    git clone git@gits-15.sys.kth.se:inda-25/<KTH_ID>-task-15.git
    ```
3) Add the upstream for `task-15-<KTH_ID>` to your local repository.
    ```sh
    git remote add plus git@github.com:IndaPlus25/task-15-<KTH_ID>.git
    ```
4) Navigate into your newly created repository and start writing.

To replicate the conditions of Rust on Kattis, see the [Kattis compiler settings for Rust](https://open.kattis.com/languages/rust).
