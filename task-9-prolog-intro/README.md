...
1) Create a repository named `task-9-<KTH_ID>`.
2) Clone your regular assignment repository.
```sh
git clone git@gits-15.sys.kth.se:inda-25/<KTH_ID>-task-9.git
```
1) Add the upstream for `task-9-<KTH_ID>` to your local repository.
```sh
git remote add plus git@github.com:IndaPlus25/task-9-<KTH_ID>.git
```
1) Create one `.pl` file for the entire assignment.

For help with settling, see `./examples`.
