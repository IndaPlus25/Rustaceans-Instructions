...
1) Create a repository named `task-16-<KTH_ID>`.
2) Clone your regular assignment repository.
    ```sh
    git clone git@gits-16.sys.kth.se:inda-25/<KTH_ID>-task-16.git
    ```
3) Add the upstream for `task-16-<KTH_ID>` to your local repository.
    ```sh
    git remote add plus git@github.com:IndaPlus25/task-16-<KTH_ID>.git
    ```
...
The minimal feature requirements for your database management system are:
- insertion
- deletion
- select all (list all)
...
- Structured data, aka. tabled structure. SQL databases follow this format. 
  - Implement a primitive version of a SQL engine.
